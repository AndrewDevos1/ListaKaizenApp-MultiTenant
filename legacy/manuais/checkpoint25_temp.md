...
Enquanto isso, deixo o texto pronto para a issue:

  Título:
  Checklist offline: passos de validação pós‑deploy

  Descrição:

  ## Contexto
  Correções aplicadas para cache e sync offline de checklists.

  ## Próximos passos (validação)
  1. Hard reload (Ctrl+Shift+R) para atualizar o service worker.
  2. Com internet: abrir `/admin/checklists` e um checklist específico para aquecer o cache.
  3. Ativar modo offline (DevTools → Network → Offline).
  4. Marcar itens; verificar mensagem “Sem conexão. Marcação salva localmente e será sincronizada.”
  5. Voltar online e confirmar sincronização (itens marcados e progresso atualizados).

  ## Observação
  O modo offline depende de abrir o checklist ao menos uma vez online para cachear os dados.

  Quer que eu rode o gh issue create assim que você fizer o login?
