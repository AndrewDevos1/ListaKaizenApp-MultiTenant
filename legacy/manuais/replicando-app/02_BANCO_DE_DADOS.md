...
**Estrutura do campo `layout` (JSON):**
```json
{
  "by_role": {
    "SUPER_ADMIN": {
      "groups": [
        { "id": "visao_geral", "title": "VISÃO GERAL", "items": ["admin_dashboard", "admin_global"] },
        { "id": "gestao", "title": "GESTÃO", "items": ["admin_listas_compras", "admin_submissoes"] }
      ],
      "hidden_items": ["admin_catalogo_global"],
      "item_labels": { "admin_dashboard": "Painel" }
    },
    "ADMIN": { "groups": [...], "hidden_items": [...] },
    "COLLABORATOR": { "groups": [...], "hidden_items": [...] }
  },
  "default": { "groups": [...], "hidden_items": [] }
}
```

**Papel no sistema:**
- Registro único (singleton) — apenas 1 linha na tabela
- Armazena o layout da navbar customizado por role pelo SUPER_ADMIN
- `by_role` contém layouts separados por perfil (SUPER_ADMIN, ADMIN, COLLABORATOR, SUPPLIER)
- `default` é o fallback quando não existe layout específico para uma role
- Cada layout define: grupos (categorias), ordem dos itens, itens ocultos e rótulos customizados

**Nota técnica (SQLAlchemy):**
- Coluna JSON com mutação in-place requer `flag_modified(instance, 'layout')` antes do `commit()`, pois SQLAlchemy não detecta mudanças em objetos JSON mutados — só detecta reatribuição de referência
