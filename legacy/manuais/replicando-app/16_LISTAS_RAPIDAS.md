...
**Desarquivamento:**
- Ao desarquivar, o `status` é **resetado para `RASCUNHO`**
- `submetido_em`, `respondido_em`, `mensagem_admin` e `admin_id` são limpos
- O colaborador pode editar e resubmeter normalmente após o desarquivamento
- ⚠️ Este comportamento é intencional: desarquivar significa "reabrir para revisão"
...
### MinhasListasRapidas.tsx (Colaborador — `/collaborator/minhas-listas-rapidas`)

- Lista todas as listas rápidas do colaborador autenticado
- Badge de status colorido por valor lowercase: `rascunho`, `pendente`, `aprovada`, `rejeitada`
- Botão "Editar" visível apenas quando `status === 'rascunho'`
- Botão "Nova Lista" → navega para criar
- Deletar lista (com confirmação)

### EditarListaRapida.tsx (Colaborador — `/collaborator/lista-rapida/:id`)

- Redireciona automaticamente se `status === 'aprovada'` (somente leitura)
- Permite editar nome, descrição, itens e prioridades quando `status === 'rascunho'`
- Botão "Submeter para Aprovação" visível apenas quando `status === 'rascunho'`
- ⚠️ Comparações de status devem usar **lowercase** (`'rascunho'`, `'aprovada'`) pois a API retorna valores em minúsculo

### DetalhesListaRapida.tsx (Colaborador)

- Visualização somente leitura da lista e seus itens
- Exibe mensagem do admin (aprovação/rejeição)
- Badge de status e prioridade por item
...
| `backend/kaizen_app/services.py` | Lógica de criação, submissão, aprovação, rejeição, arquivamento |
...
| `frontend/src/features/colaborador/MinhasListasRapidas.tsx` | Lista de listas do colaborador |
| `frontend/src/features/colaborador/EditarListaRapida.tsx` | Edição e submissão de lista |
| `frontend/src/features/colaborador/DetalhesListaRapida.tsx` | Visualização de lista (somente leitura) |
| `frontend/src/features/colaborador/CriarListaRapida.tsx` | Criação de nova lista pelo colaborador |
