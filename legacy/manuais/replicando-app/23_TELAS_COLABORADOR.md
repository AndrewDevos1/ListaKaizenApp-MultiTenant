...
- Aceita: números, decimais (vírgula ou ponto), expressões de adição (ex: `10+5+3`)
- `parseQuantidadeInput()` normaliza decimais (`0,6 → 0.6`) e avalia somas
- `type="text"` com `inputMode="text"` e `pattern="[0-9+.,]*"`

**Handlers de teclado/foco:**

| Handler | Evento | Comportamento |
|---------|--------|---------------|
| `handleQuantidadeKeyDown` | `onKeyDown` | Desktop: Enter avalia expressão e move foco ao próximo; Tab move ao anterior |
| `handleQuantidadeKeyUp` | `onKeyUp` | Android fix: IME envia `key='Unidentified'` no keydown, mas `key='Enter'` correto no keyup |
| `handleQuantidadeBlur` | `onBlur` | Avalia expressão e normaliza decimais ao perder foco (cobre: toque no próximo campo, "Done" no teclado mobile, Tab) |

**Cobertura de cenários:**

| Cenário | Handler ativo |
|---------|---------------|
| Desktop: `5+5+3` + Enter | `onKeyDown` → `13` |
| Android: `5+5+3` + Enter | `onKeyUp` → `13` |
| Android: `5+5+3` + toca próximo campo | `onBlur` → `13` |
| Mobile: `0,6` + toca próximo campo | `onBlur` → `0.6` |

- **Navegação desktop:** Tab → item anterior; Enter → item seguinte (só `onKeyDown` move foco, evita dupla navegação)
