...
## Inputs de Quantidade no Mobile (Android IME)

Campos de quantidade de estoque (EstoqueListaCompras, ListaEstoque, EstoqueLista) têm comportamento especial para suportar o teclado virtual Android.

### Problema: Android IME e `onKeyDown`

No Android, o teclado virtual usa Input Method Editor (IME). O evento `keydown` chega com `event.key = 'Unidentified'`, então o Enter nunca é detectado via `onKeyDown`. O evento `keyup` sim recebe `key = 'Enter'` corretamente.

### Solução: três handlers complementares

```tsx
// 1. Desktop: Enter → avalia + move foco; Tab → move foco (anterior)
const handleQuantidadeKeyDown = (event, estoqueId) => { ... };

// 2. Android: captura Enter no keyup (IME-safe)
const handleQuantidadeKeyUp = (event, estoqueId) => {
    if (event.key !== 'Enter') return;
    const parsed = parseQuantidadeInput(event.currentTarget.value);
    if (parsed !== null) handleQuantityChange(estoqueId, String(parsed));
    // Não chama focusRelativeQuantidadeInput → evita dupla navegação no desktop
};

// 3. Blur: cobre "toque no próximo campo" e normaliza decimais
const handleQuantidadeBlur = (event, estoqueId) => {
...
### `parseQuantidadeInput` (utils/quantityParser.ts)
